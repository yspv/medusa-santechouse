...
import { FocusModal, Button, CommandBar, toast } from "@medusajs/ui";
...
import {
  useBrandImageMutations,
  brandsQueryKeys,
} from "../../../../../hooks/api/brands";

type BrandMediaModalProps = {
  brandId: string;
  existingImages: BrandImageDTO[];
};

export const BrandMediaModal = ({
  brandId,
  existingImages,
}: BrandMediaModalProps) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (thumbnailChanged) {
        const updates = [
          { id: currentThumbnailId, type: "thumbnail" as const },
        ];
      ...
      setOpen(false);
      resetModalState();
      toast.success(t("brands.media.saveSuccess"));
    } catch (error) {
      toast.error(t("brands.media.saveError"));
    }
  };
      ...
      <FocusModal.Content>
        ...
        <FocusModal.Body className="flex h-full overflow-hidden">
          ...
          <CommandBar open={selectedImageIds.size > 0}>
            <CommandBar.Bar>
              <CommandBar.Value>
                {t("brands.media.selected", { count: selectedImageIds.size })}
              </CommandBar.Value>
              <CommandBar.Seperator />
              <CommandBar.Command
                action={handleSetAsThumbnail}
                label={t("brands.media.setAsThumbnail")}
                shortcut="t"
